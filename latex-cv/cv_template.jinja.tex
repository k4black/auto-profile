...
%    \homepage{(( bio.website ))}
  %  \twitter{@twitterhandle}
    \linkedin{(( bio.linkedin_name ))}
    \github{(( bio.github_name ))}
  ...
  %\end{fullwidth}
  \makecvfooter
      {Made using my \ihref[\faGithub]{((project_url))}{((project_name))} project}
      {}
      {Updated \ifcase\month\or January\or February\or March\or April\or May\or June\or July\or August\or September\or October\or November\or December\fi\ \the\year}%
  ...
  \begin{paracol}{2}
    ...
    (#- for internship in internships_list -#)
        \ihref{(( internship.url ))}{
            (( _process_text_to_latex(internship.company)
               ~ (', ' ~ _process_text_to_latex(internship.role) if internship.role is defined else '') ))
        }%
        (( '(' ~ internship.date ~ ')' ))%
...
%    \printbibliography[heading=pubtype,title={\printinfo{\faUsers}{Conference Proceedings}},type=inproceedings]
    ...
    \cvsection{Summary}
    \vspace{0.6em} % TODO: fix
    \textbf{(( _process_text_to_latex(short_summary) ))}\par%
%    \smallskip%
%    (( summary ))
    \vspace{-0.4em} % TODO: fix
    \cvbox{
        \begin{itemize}
            \setlength\itemsep{0.2em}
            (#- for summary_element in summary #)
                \item (( _process_text_to_latex(summary_element) ))
            (#- endfor #)
        \end{itemize}
    }%
    \medskip
